...
@Component({
  selector: 'app-aduan-perhatian-list',
  templateUrl: './aduan-perhatian-list.page.html',
  styleUrls: ['./aduan-perhatian-list.page.scss'],
})
export class AduanPerhatianListPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.aduans$ = this.aduanService.getAduans().pipe(
      map((aduans) => {
        ...
        return aduans.filter((res) => res.status_code === 'P');
      })
    );
    if (this.aduans$) {
      this.haveInfo = true;
